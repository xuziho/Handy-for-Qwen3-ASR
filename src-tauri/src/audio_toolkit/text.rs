use crate::settings::TermCorrectionRule;
...
pub fn apply_term_corrections(text: &str, rules: &[TermCorrectionRule]) -> String {
    if rules.is_empty() || text.is_empty() {
        return text.to_string();
    }

    let mut normalized_rules: Vec<&TermCorrectionRule> = rules
        .iter()
        .filter(|r| r.enabled && !r.canonical.trim().is_empty() && !r.aliases.is_empty())
        .collect();
    ...
    normalized_rules.sort_by(|a, b| {
        b.priority
            .cmp(&a.priority)
            .then_with(|| b.canonical.len().cmp(&a.canonical.len()))
    });

    let mut output = text.to_string();

    for rule in normalized_rules {
        let replacement = rule.canonical.trim();
        for alias in &rule.aliases {
            let alias = alias.trim();
            if alias.is_empty() || alias == replacement {
                continue;
            }

            let escaped = regex::escape(alias);
            let base_pattern = if rule.whole_word {
