...
use crate::settings::{
    self, get_settings, AsrBackend, AutoSubmitKey, ClipboardHandling, KeyboardImplementation,
    LLMPrompt, OverlayPosition, PasteMethod, ShortcutBinding, SoundTheme, TypingTool,
    APPLE_INTELLIGENCE_PROVIDER_ID,
};
use crate::tray;

fn qwen_model_urls(base_url: &str) -> Vec<String> {
...
#[tauri::command]
#[specta::specta]
pub fn change_asr_backend_setting(app: AppHandle, backend: String) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.asr_backend = match backend.as_str() {
        "local" => AsrBackend::Local,
        "qwen_http" => AsrBackend::QwenHttp,
        _ => return Err(format!("Invalid ASR backend: {}", backend)),
    };
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_qwen_base_url_setting(app: AppHandle, base_url: String) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
...
#[tauri::command]
#[specta::specta]
pub fn change_qwen_api_key_setting(app: AppHandle, api_key: String) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
...
#[tauri::command]
#[specta::specta]
pub fn change_qwen_model_id_setting(app: AppHandle, model_id: String) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.qwen_model_id = model_id;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_qwen_timeout_sec_setting(app: AppHandle, timeout_sec: u64) -> Result<(), String> {
    if timeout_sec == 0 {
        return Err("Timeout must be greater than 0 seconds".to_string());
    }

    let mut settings = settings::get_settings(&app);
    settings.qwen_timeout_sec = timeout_sec;
    settings::write_settings(&app, settings);
    Ok(())
}
...
pub async fn fetch_qwen_models(base_url: String, api_key: Option<String>) -> Result<Vec<String>, String> {
    ...
    for url in urls {
        ...
        match request.send().await {
            Ok(response) => {
                ...
                if models.is_empty() {
                    last_error = format!("{} returned no model IDs", url);
                    continue;
                }
    ...
    Err(if last_error.is_empty() {
        "Failed to fetch models from Qwen endpoint".to_string()
    } else {
        last_error
    })
}
...
#[tauri::command]
#[specta::specta]
pub fn change_debug_log_full_post_process_prompt_setting(
    app: AppHandle,
    enabled: bool,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.debug_log_full_post_process_prompt = enabled;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn update_term_corrections(
    app: AppHandle,
    rules: Vec<settings::TermCorrectionRule>,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.term_corrections = rules;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_post_process_include_term_corrections_setting(
    app: AppHandle,
    enabled: bool,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.post_process_include_term_corrections = enabled;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_post_process_term_corrections_prompt_template_setting(
    app: AppHandle,
    template: String,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.post_process_term_corrections_prompt_template = template;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_replace_sentence_period_with_space_setting(
    app: AppHandle,
    enabled: bool,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.replace_sentence_period_with_space = enabled;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_strip_language_markers_setting(app: AppHandle, enabled: bool) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.strip_language_markers = enabled;
    settings::write_settings(&app, settings);
    Ok(())
}
