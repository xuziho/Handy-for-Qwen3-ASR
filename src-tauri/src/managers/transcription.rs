use crate::audio_toolkit::{
    apply_custom_words, apply_term_corrections, filter_transcription_output,
};
...
struct QwenTranscriptionResponse {
    text: String,
}

fn normalize_qwen_language(language: &str) -> Option<String> {
    match language {
        "" | "auto" => None,
...
fn clean_qwen_transcription_text(raw: &str) -> String {
    let mut text = raw.trim().to_string();

    if let Some((_, tail)) = text.split_once("<asr_text>") {
...
impl TranscriptionManager {
    ...
    fn transcribe_via_qwen_http(&self, audio: &[f32], settings: &AppSettings) -> Result<String> {
        let wav = samples_to_wav_bytes(audio, 16_000)?;

        let model = settings.qwen_model_id.trim();
        if model.is_empty() {
            return Err(anyhow::anyhow!(
                "Qwen backend selected but model id is empty"
            ));
        }

        let endpoint = format!(
            "{}/audio/transcriptions",
            settings.qwen_base_url.trim_end_matches('/')
        );
        ...
        if let Some(language) = normalize_qwen_language(&settings.selected_language) {
        ...
        let api_key = if settings.qwen_api_key.trim().is_empty() {
            "EMPTY"
        } else {
        ...
        if !response.status().is_success() {
            ...
            return Err(anyhow::anyhow!(
                "Qwen transcription request failed with {}: {}",
                status,
                body
            ));
        }
    ...
    pub fn initiate_model_load(&self) {
        let settings = get_settings(&self.app_handle);
        if settings.asr_backend == AsrBackend::QwenHttp {
            return;
        }
    ...
    pub fn transcribe(&self, audio: Vec<f32>) -> Result<String> {
        ...
        if settings.asr_backend == AsrBackend::QwenHttp {
            let raw_result = self.transcribe_via_qwen_http(&audio, &settings)?;
            ...
            let term_corrected_result = if !settings.term_corrections.is_empty() {
                apply_term_corrections(&corrected_result, &settings.term_corrections)
            } else {
                corrected_result
            };

            let final_result = filter_transcription_output(
                &term_corrected_result,
            ...
            info!(
                "Qwen HTTP transcription completed in {}ms",
                (et - st).as_millis()
            );

            self.maybe_unload_immediately("qwen_http transcription");
