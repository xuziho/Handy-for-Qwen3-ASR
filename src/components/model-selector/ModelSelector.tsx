...
const ModelSelector: React.FC<ModelSelectorProps> = ({ onError }) => {
  ...
  const [isQwenChecking, setIsQwenChecking] = useState(false);
  const [qwenServiceOnline, setQwenServiceOnline] = useState(false);
  const [qwenDetectedModels, setQwenDetectedModels] = useState<string[]>([]);
  ...
  const qwenModelId = getSetting("qwen_model_id") || "qwen_http";

  const displayModelId = pendingModelId || currentModel;

  useEffect(() => {
    const checkQwenService = async () => {
      if (asrBackend !== "qwen_http") return;

      setIsQwenChecking(true);
      try {
        const result = await commands.fetchQwenModels(
          qwenBaseUrl,
          qwenApiKey.trim() ? qwenApiKey.trim() : null,
        );

        if (result.status === "ok") {
          setQwenServiceOnline(true);
          setQwenDetectedModels(result.data);
          setModelError(null);
        } else {
          setQwenServiceOnline(false);
          setQwenDetectedModels([]);
          setModelError(result.error || t("modelSelector.modelError"));
        }
      } catch {
        setQwenServiceOnline(false);
        setQwenDetectedModels([]);
        ...
        setIsQwenChecking(false);
      }
    };

    void checkQwenService();
  }, [asrBackend, qwenBaseUrl, qwenApiKey, t]);

  // Check model status when currentModel changes
  useEffect(() => {
    const checkStatus = async () => {
      if (asrBackend === "qwen_http") {
        setModelStatus(qwenServiceOnline ? "ready" : "error");
        return;
      }
    ...
    checkStatus();
  }, [asrBackend, currentModel, qwenServiceOnline]);
  ...
  const getModelDisplayText = (): string => {
    if (asrBackend === "qwen_http") {
      ...
      if (!qwenServiceOnline) {
        return t("modelSelector.qwenOffline", {
          modelName: qwenModelId,
          defaultValue: "Qwen offline: {{modelName}}",
        });
      }

      const activeModel =
        qwenDetectedModels.find((id) => id === qwenModelId) || qwenModelId;

      return t("modelSelector.qwenActive", {
        modelName: activeModel,
        defaultValue: "Qwen: {{modelName}}",
      });
    }
  ...
  const getDisplayStatus = (): ModelStatus => {
    if (asrBackend === "qwen_http") {
      if (isQwenChecking) return "loading";
      return qwenServiceOnline ? "ready" : "error";
    }
  ...
  return (
    <>
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        {/* Model Dropdown */}
        {asrBackend !== "qwen_http" && showModelDropdown && (
          <ModelDropdown
            models={models}
